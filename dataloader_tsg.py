...
class Dataset(data.Dataset):
    ...
    def collate_func(self, batch, split):
        ...
        for sample in batch:
            # fetch image
            tmp_fc, tmp_att, tmp_seq, tmp_rela, ix, it_pos_now, tmp_wrapped = sample
            if tmp_wrapped:
                wrapped = True
        ...
        fc_batch, att_batch, rela_batch, label_batch, gts, infos = zip(
            *sorted(
                zip(fc_batch, att_batch, rela_batch, label_batch, gts, infos),
                key=lambda x: 0,
                reverse=True,
            )
        )
        ...
        for i in range(len(att_batch)):
            data["enc_self_masks"][
                i, : att_batch[i].shape[0], : att_batch[i].shape[0]
            ] = 1
            ...
            for j in range(rela_len[i]):
                data["enc_self_masks"][
                    i,
                    np.int32(rela_info[i][j][0]),
                    att_batch[i].shape[0] + attr_len[i] + j,
                ] = 1
                data["enc_self_masks"][
                    i,
                    att_batch[i].shape[0] + attr_len[i] + j,
    ...
    def __getitem__(self, index):
        ...
        return (fc_feat, att_feat, seq, rela_data, ix, it_pos_now, wrapped)
