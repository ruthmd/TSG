...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, src_tgt_mask, tgt_mask):
        ...
        return self.decode(self.encode(src, src_mask), src_tgt_mask, tgt, tgt_mask)
...
class TSGModel(AttModel):
    ...
    def _prepare_feature_forward(
        ...
        for i in range(len(rela_len)):
            att_feats[i, int(att_len[i]) : int(att_len[i] + attr_len[i]), :] = (
                rela_feats[i, : int(attr_len[i]), :]
                + rela_feats[i, int(attr_len[i]) : int(2 * attr_len[i]), :]
            )
            att_feats[
                i,
                int(att_len[i] + attr_len[i]) : int(
                    att_len[i] + attr_len[i] + rela_len[i]
                ),
                :,
            ] = rela_feats[
                i, int(2 * attr_len[i]) : int(2 * attr_len[i] + rela_len[i]), :
            ]
    ...
    def _sample(
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(
                it, att_feats, att_masks, state, output_logsoftmax=output_logsoftmax
            )
    ...
    def _sample_beam(
        ...
        self.done_beams = self.beam_search(
            state, logprobs, att_feats, att_masks, opt=opt
        )
