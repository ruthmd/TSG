...
class EncoderDecoder(nn.Module):
    ...
    def forward(
        self, src, tgt, src_mask, src_tgt_mask, tgt_mask, rela_len, attr_len, att_len
    ):
    ...
    def decode(self, memory, src_tgt_mask, tgt, tgt_mask, rela_len, attr_len, att_len):
        memory_att, memory_attr, memory_rela, mask_att, mask_attr, mask_rela = (
            divide_memory(memory, rela_len, attr_len, att_len)
        )
        out = self.decoder(
            self.tgt_embed(tgt),
            memory_att,
...
def divide_memory(memory, rela_len, attr_len, att_len):
    ...
    for i in range(len(rela_len)):
        ...
        memory_att[i * seq_per_img : (i + 1) * seq_per_img, : att_len[i], :] = memory[
            i * seq_per_img : (i + 1) * seq_per_img, : att_len[i], :
        ]
        memory_attr[i * seq_per_img : (i + 1) * seq_per_img, : attr_len[i], :] = memory[
            i * seq_per_img : (i + 1) * seq_per_img,
            att_len[i] : attr_len[i] + att_len[i],
            :,
        ]
        memory_rela[i * seq_per_img : (i + 1) * seq_per_img, : rela_len[i], :] = memory[
            i * seq_per_img : (i + 1) * seq_per_img,
            attr_len[i] + att_len[i] : attr_len[i] + att_len[i] + rela_len[i],
            :,
        ]
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        x_att = self.sublayer[1](
            x, lambda x: self.src_attn_att(x, memory_att, memory_att, mask_att)
        )
        x_attr = self.sublayer[2](
            x, lambda x: self.src_attn_attr(x, memory_attr, memory_attr, mask_attr)
        )
        x_rela = self.sublayer[3](
            x, lambda x: self.src_attn_rela(x, memory_rela, memory_rela, mask_rela)
        )
...
class TSGMModel3(AttModel):
    ...
    def _prepare_feature_forward(
        ...
        for i in range(len(rela_len)):
            rela_type = self.type_embed(
                torch.ones(
                    att_feats[i, att_len[i] : att_len[i] + attr_len[i], :].shape[0:1],
                    dtype=torch.int32,
                ).cuda()
            )
            att_feats[i, att_len[i] : att_len[i] + attr_len[i], :] = (
                rela_feats[i, : attr_len[i], :]
                + rela_feats[i, attr_len[i] : 2 * attr_len[i], :]
                + rela_type
            )
            att_feats[
                i, att_len[i] + attr_len[i] : att_len[i] + attr_len[i] + rela_len[i], :
            ] = rela_feats[
                i, 2 * attr_len[i] : 2 * attr_len[i] + rela_len[i], :
            ] + self.type_embed(
                torch.ones(
                    att_feats[
                        i,
    ...
    def core(self, it, memory, state, mask, rela_len, attr_len, att_len):
        ...
        out = self.model.decode(
            ...
            mask,
            ys,
            subsequent_mask(ys.size(1), self.all_former).to(memory.device),
            rela_len,
            attr_len,
            att_len,
        )
    ...
    def _sample_beam(
        ...
        att_feats, att_masks = utils.repeat_tensors(beam_size, [att_feats, att_masks])
